...
<body>
	...
	<script type="text/javascript">
		...
		function reload_profile() {
			$.ajax({
				url: "<?= site_url('AdminController/profileInfo/' . $admin->id_admin) ?>",
		...
		function reload_form() {
			$.ajax({
				url: "<?= site_url('AdminController/profileSetting/' . $admin->id_admin) ?>",
				beforeSend: function(f) {
				...
				$.ajax({
					...
					success: function(data) {
						if (data == undefined || email == '') {
							...
							$.ajax({
								url: "<?= site_url('AdminController/updateProfile') ?>",
								type: 'POST',
								data: {
									[csrfToken]: csrfHash,
									id: $("#id").val(),
