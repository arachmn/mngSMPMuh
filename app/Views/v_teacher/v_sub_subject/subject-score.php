...
<body>
                ...
                <div class="page-header">
                    <div class="row">
                        <div class="col-md-6 col-sm-12">
                            <div class="title">
                                <h4><?= $subject['subject_name'] ?> | <?= $subject['class'] ?> | <?= $semester ?></h4>
                            </div>
                ...
                <div class="card-box mb-30">
                    <div class="pd-20">
                        ...
                        <div class="d-inline" style="float: right;">
                            <button class="btn btn-outline-primary" id="printScore" name="printScore" onclick="">
                                <i class="bi bi-printer-fill" style="font-size: 20px;"></i>
                            </button>
                        </div>
                    </div>

                    <div class="mt-3" id="score_table" style="overflow: auto;">
                    </div>
                </div>
            </div>
    ...
    <script type="text/javascript">
        ...
        function reload_table(id, semester, num) {
            $.ajax({
                url: "<?= site_url('TeacherController/subjectScoreTable/') ?>" + id + '/' + semester,
                beforeSend: function(f) {
                    $('#score_table').html(loader);
                },
                success: function(data) {
                    $('#score_table').html(data);
                    if (num != '' && num != 1) {
                        $([document.documentElement, document.body]).animate({
                            scrollTop: $("#tr" + (num - 1)).offset().top
                        }, 100);
                        $('.modal').each(function() {
                            $(this).modal('hide');
                        });
        ...
        function printScore(kurikulum) {
            if (kurikulum == 'Merdeka') {
                url = "<?= site_url('TeacherController/printScoreMerdeka/' . $id . '/' . $semester) ?>";

            } else if (kurikulum == 'K-13') {
                url = "<?= site_url('TeacherController/printScoreK13/' . $id . '/' . $semester) ?>";
            }
            window.open(
                url,
                '_blank'
