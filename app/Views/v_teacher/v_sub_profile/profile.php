...
<body>
					...
					<div class="row">
						<div class="col-md-6 col-sm-12">
							...
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
									<li class="breadcrumb-item"><a href="<?= site_url('teacher') ?>">Home</a></li>
							...
							<div class="profile-tab height-100-p">
								<div class="tab height-100-p">
									<ul class="nav nav-tabs customtab" role="tablist">
										<li class="nav-item">
											<a class="nav-link active" data-toggle="tab" href="#timeline" role="tab">Jadwal</a>
										</li>
										<li class="nav-item">
											<a class="nav-link" data-toggle="tab" href="#setting" role="tab">Pengaturan Profile</a>
										</li>
									</ul>
	...
	<script type="text/javascript">
		...
		function reload_profile() {
			$.ajax({
				url: "<?= site_url('TeacherController/profileInfo/' . $idTeacher) ?>",
		...
		function reload_timetable() {
			$.ajax({
				url: "<?= site_url('TeacherController/profileTimetable/' . $idTeacher) ?>",
				beforeSend: function(f) {
					$('.profile-timeline').html(loader);
				},
				success: function(data) {
		...
		function reload_form() {
			$.ajax({
				url: "<?= site_url('TeacherController/profileSetting/' . $idTeacher) ?>",
				beforeSend: function(f) {
		...
		function updateProfile() {
			...
			if (phone_number != '' && $.isNumeric(phone_number) == false) {
				...
				$("#phone_number_feedback").html('Phone Number harus berupa angka');
				valError = true;
			}
			...
			if (phone_number != '' && $.isNumeric(phone_number) == true && phone_number.toString().length < 7) {
				$("#div_phone_number").attr("class", "form-group has-danger");
				$("#phone_number").attr("class", "form-control form-control-lg form-control-danger");
				$("#phone_number_feedback").html('Phone Number minimal 7 digit angka');
				valError = true;
			}
			...
			$.ajax({
				url: "<?= site_url('TeacherController/updateProfile') ?>",
				type: 'POST',
				data: {
					[csrfToken]: csrfHash,
					id: $("#id").val(),
